...
import { Link as LinkIcon, Timer as TimerIcon } from "lucide-react";
import { Badge, Button, Card, CardContent, CardHeader, CardTitle, ProgressBar } from "../ui.jsx";
import { STORAGE_KEY } from "../../data/plan.jsx";
...
const useCountdown = (sec, running) => {
  const [seconds, setSeconds] = useState(sec);
  useEffect(() => setSeconds(sec), [sec]);
  useEffect(() => {
    if (!running) return;
...
export const Timer = ({ preset = 40, label = "Work" }) => {
  ...
  const [seconds, setSeconds] = useCountdown(input, running);
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-gray-200 p-3">
      ...
      <input className="w-20 rounded-lg border border-gray-300 px-2 py-1 text-sm" type="number" min={5}
             value={input} onChange={(e) => setInput(Number(e.target.value || 0))} />
      ...
      <CardContent>
        ...
        <div className="mt-5 rounded-xl bg-gray-50 p-3 text-xs text-gray-700">
          ...
          <ul className="list-inside list-disc space-y-1">
            <li>10-min warm-up: easy skipping + dynamic mobility.</li>
            <li>Stop 1–2 reps before form breaks (except last set).</li>
            <li>Nasal breathing on easy work; mouth for sprints.</li>
            <li>Water + pinch of salt before your night meal workout.</li>
          </ul>
        </div>
      </CardContent>
    </Card>
